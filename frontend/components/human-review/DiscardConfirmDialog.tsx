...
export function DiscardConfirmDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink/35 p-4 sm:items-center" role="presentation">
      <section ref={dialogRef} onKeyDown={handleKeyDown} role="dialog" aria-modal="true" aria-labelledby="discard-dialog-title" aria-describedby="discard-dialog-description" className="w-full max-w-lg rounded-[8px] border border-line bg-white p-6 shadow-soft">
        <div className="flex items-start gap-3">
          <span className="rounded-full bg-coral/15 p-2 text-coral"><AlertTriangle size={18} aria-hidden="true" /></span>
          <div>
            <h3 id="discard-dialog-title" className="text-lg font-semibold text-ink">Confirmar descarte</h3>
            <p id="discard-dialog-description" className="mt-1 text-sm leading-6 text-ink/60">El descarte se aplicará solo al confirmar y quedará auditado por el servidor.</p>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          ...
          <button type="button" onClick={onConfirm} disabled={pending || confirmationBlocked || !reason.trim()} className="focus-ring rounded-[8px] bg-coral px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">{pending ? "Confirmando..." : "Descartar caso"}</button>
        </div>
      </section>
    </div>
  );
